...
| Export | Shape |
|---|---|
| `site` | name, tagline, location, serviceArea ("Across Lagos, and further afield by arrangement", from the brand FAQ), phone/phoneIntl, emails[] (currently one: info@brandfirstmedia.com; rendered via `EmailLink`, which breaks long addresses at the @), social[], formEndpoint (null ⇒ mailto fallback), whatsapp (wa.me digits), whatsappDisplay, whatsappMessage (pre-filled enquiry text used by every WhatsApp link) |
| `home` | hero{headlineLines, sub, primary, secondary}, intro, group, why{highlights[]}, industries{sectors[]}, cta |
| `services[]` (9, three per brand) | { n, slug, brand, image, title, card, body, includes[] } — `brand` ∈ brandfirst / room16 / aso-igbalode |
...
| Module | Shape |
|---|---|
| `config.js` | `SITE_URL` (default `https://brandfirstmedia.com`, env `VITE_SITE_URL`), `SITE_HOST`, `absolute(path)`, `LOCALE` en_NG, `LANGUAGE` en-NG |
| `pages.js` | `PAGES[path]` = { title, description, og (image name in `public/og/`, 1200×630 JPEG), ogAlt, type (schema.org page type), trail[[name, path]] (breadcrumb after Home), brand? }; `NOT_FOUND` (noindex); `PUBLIC_PATHS`; `pageFor(pathname)` |
| `schema.js` | `schemaFor(page)` → one JSON-LD `@graph`: Organization + ProfessionalService (Brandfirst Media: name, url, logo, telephone, email, city/country address, areaServed Lagos, two ContactPoints incl. WhatsApp, Instagram sameAs, `brand` → the two Brand nodes), Brand ×2 (ROOM16, Aṣọ Ìgbàlódé), WebSite, the page (WebPage / AboutPage / ContactPage / CollectionPage), BreadcrumbList (inner pages), Service (all nine on /services, three per brand page; `@id` = `/services#slug`), FAQPage (brand pages, from their visible Q&A). No street address, hours, reviews, ratings or prices. |
