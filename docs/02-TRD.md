...
- **Production: Cloudflare**, live at **https://brandfirstmedia.com** (and `www.`), since September 2026. The site is an assets-only **Worker with static assets** named `brandfirst-media` (config: `brandfirst-media/wrangler.jsonc`), in the Cloudflare account that also holds the `brandfirstmedia.com` zone.
  ...
  - **Worker for verification files only.** `worker/index.js` runs just for paths matching `assets.run_worker_first` (`/google*.html`); all other requests go straight to the files. Google Search Console's HTML-file verification does not follow redirects, and `html_handling` redirects `/x.html` to `/x`, so the Worker serves the file at its exact `.html` URL with a 200. The file itself is `public/googleca6914e6c961c46d.html` (Search Console, URL-prefix property).
